...
export class ParserRepair {
    ...
    static insertMissingRightParens(tokens, messages) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            for (let j = i + 1; j < tokens.length; j++) {
                ...
                if (sawLeftParen &&
                    ...
                    messages.push(`Inserted missing ')' before '${current.value}' at file (${current.line}:${current.column}).`);
                    return;
                }
    ...
    static insertMissingRightBraces(tokens, messages) {
        let balance = 0;
        ...
        for (let i = 0; i < balance; i++) {
            ...
            messages.push(`Inserted missing '}' before '$' at file (${eop.line}:${eop.column}).`);
        }
    }
    // Creates a synthetic token using the position of a nearby token.
    static syntheticToken(type, value, near) {
        return new Token(type, value, near.line, near.column);
    }
}
